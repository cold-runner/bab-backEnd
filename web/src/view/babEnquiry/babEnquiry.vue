  ...
  <div>
    <div class="gva-search-box">
      <el-form :inline="true" :model="searchInfo" class="demo-form-inline" @keyup.enter="onSubmit">
        ...
        <el-form-item label="用户名字">
          <el-input v-model="searchInfo.firstName" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="用户姓氏">
          <el-input v-model="searchInfo.lastName" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="用户要咨询的城市">
        ...
        <el-form-item label="用户微信号">
          <el-input v-model="searchInfo.wechatNo" placeholder="搜索条件" />

        </el-form-item>
        <el-form-item label="用户邮箱地址">
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="用户名字" prop="firstName" width="120" />
        <el-table-column align="left" label="用户姓氏" prop="lastName" width="120" />
        <el-table-column align="left" label="用户要咨询的城市" prop="city" width="200" />
        <el-table-column align="left" label="用户手机号" prop="phone" width="120" />
        <el-table-column align="left" label="用户微信号" prop="wechatNo" width="120" />
        <el-table-column align="left" label="用户邮箱地址" prop="email" width="120" />
        <el-table-column align="left" label="用户留言" prop="message" width="120" />
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBabEnquiryFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getBabEnquiryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteBabEnquiryByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateBabEnquiryFunc = async(row) => {
  const res = await findBabEnquiry({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
...
// 删除行
const deleteBabEnquiryFunc = async(row) => {
  const res = await deleteBabEnquiry({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
