  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="center" label="资讯内容" prop="content" width="1000" />
        <el-table-column align="center" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBabNewsFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="120px">
        <el-form-item label="标题:" prop="title">
          <el-input v-model="formData.title" :clearable="false" placeholder="请输入" />
        </el-form-item>
...
const getTableData = async() => {
  const table = await getBabNewsList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteBabNewsByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateBabNewsFunc = async(row) => {
  const res = await findBabNews({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    const resp = res.data.renewsInfo
    ...
    formData.value.content = resp.content
    formData.value.title = resp.title
...
// 删除行
const deleteBabNewsFunc = async(row) => {
  const res = await deleteBabNews({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       multiForm.append('title', formData.value.title)
       multiForm.append('content', formData.value.content)
       ...
       switch (type.value) {
         case 'create':
           res = await createBabNews(multiForm)
           break
         case 'update':
           ...
           res = await updateBabNews(multiForm)
           break
         default:
           res = await createBabNews(multiForm)
           break
