  ...
  <div>
    ...
    <div class="gva-table-box">
      ...
      >
        ...
        <el-table-column align="left" label="城市英文名称" prop="name" width="120" />
        <el-table-column align="left" label="城市中文名称" prop="chineseName" width="120" />
        <el-table-column align="left" label="城市介绍" prop="introduction" width="120" />
        <el-table-column align="left" label="按钮组">
          <template #default="scope">
            <el-button type="primary" link icon="edit" class="table-button" @click="updateBabCityFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" title="弹窗操作">
      <el-form ref="elFormRef" :model="formData" label-position="right" :rules="rule" label-width="120px">
        ...
        <el-form-item label="城市介绍:" prop="introduction">
          <el-input v-model="formData.introduction" :clearable="true" placeholder="请输入" />
        </el-form-item>
        <el-form-item label="城市图片:" prop="image">
          <el-upload
...
const getTableData = async() => {
  const table = await getBabCityList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ...
  const res = await deleteBabCityByIds({ ids })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
// 更新行
const updateBabCityFunc = async(row) => {
  const res = await findBabCity({ ID: row.ID })
  type.value = 'update'
  if (res.code === 0) {
    const resp = res.data.recityInfo
...
// 删除行
const deleteBabCityFunc = async(row) => {
  const res = await deleteBabCity({ ID: row.ID })
  if (res.code === 0) {
    ElMessage({
      type: 'success',
      message: '删除成功'
...
const enterDialog = async() => {
     elFormRef.value?.validate(async(valid) => {
       ...
       switch (type.value) {
         case 'create':
           res = await createBabCity(multiForm)
           break
         case 'update':
           ...
           res = await updateBabCity(multiForm)
           break
         default:
           res = await createBabCity(multiForm)
           break
